...
def draw_loss(Loss_list,iteration,save_path):
    # 我这里迭代了200次，所以x的取值范围为(0，200)，然后再将每次相对应的准确率以及损失率附在x上
    plt.cla()
    x1 = range(1, iteration+1)
    # print(x1)
    y1 = Loss_list
    # print(y1)
    plt.title('Train loss vs. iteration', fontsize=20)
    plt.plot(x1, y1, '.-')
    plt.xlabel('iteration', fontsize=20)
    plt.ylabel('Train loss', fontsize=20)
    plt.grid()
    # save_path = mkdir(os.path.join(save_path,"Train_loss.png"), is_file=True)
